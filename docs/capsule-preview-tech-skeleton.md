...
| 页面 | 状态 | 说明 |
| --- | --- | --- |
| 首页（Homepage） | **本阶段必须补齐** | 需在 Wokiee 中配置 8 个 section（见 IA 文档）；当前 theme submodule 无 `templates/index.json` |
| 商品详情页 PDP | **已具备骨架，需推送** | `theme/templates/product-copper-kettle.liquid` 已写好，需推送到 Wokiee 并在 Shopify 后台关联 |
| 导航 / 页头 / 页脚 | **本阶段必须补齐** | 需在 Wokiee 主题编辑器中配置菜单和页脚；无需写自定义 section |
...
| 5 | 使用场景提示（Multicolumn） | **可后置** | 有图册图可配，无图可跳过 |
| 6 | 透明说明区（Rich text） | **本阶段必须配置** | 已知/待复核两列清单 |
| 7 | FAQ 摘要（Collapsible content） | **本阶段必须配置** | 4-5 条核心问答 |
...
```
sample_verified = false → 隐藏购买入口，显示到货通知表单
sample_verified = true  → 显示购买入口（样品验收后解锁）
```
...
**价格/库存/交期熔断**：
- `price` 字段在 `product-schema.json` 中为 `__PENDING_PRICING__`，Shopify draft 商品不展示价格
