...
echo "   ├─────────────────────┼──────────────────────────────┼─────────┼───────────────────────────────────────────────────┤"
echo "   │ sample_verified     │ 样品已验收                   │ boolean │ false  ← Capsule Preview 阶段锁定                 │"
...
echo "  当前模板强制 preview-only，不读取 true 值解锁购买入口"
echo "  后续如需正式销售，必须新开 CEO / board 批准后的工程变更移除 preview-only 模板"
echo ""
echo "=== 完成后验证 ==="
echo "  1. 访问 /products/${PRODUCT_HANDLE}?preview=true（draft 商品预览链接）"
echo "  2. 确认页面显示「资料预览」横幅"
echo "  3. 确认没有「加入购物车」、价格、库存、变体、评论、推荐商品、交期或可售结构化数据"
echo "  4. 确认媒体区有「资料图 / 占位图 / 待实拍」角标"
echo "  5. 确认「到货通知」表单可提交，Customer tag 为 notify-copper-bottle-pour-over-kettle"
echo ""
